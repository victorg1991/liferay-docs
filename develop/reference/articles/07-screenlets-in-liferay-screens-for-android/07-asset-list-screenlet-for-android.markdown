...
| `obcClassName` | `string` | The name of the `OrderByComparator` class to use to sort the results. Omit this property if you don't want to sort the results. Note that you can only use comparator classes that extend `OrderByComparator<AssetEntry>`. Liferay contains no such comparator classes. You must therefore create your own by extending `OrderByComparator<AssetEntry>`. To see examples of some comparator classes that extend other asset classes, [click here](https://github.com/liferay/liferay-portal/tree/master/portal-impl/src/com/liferay/portlet/asset/util/comparator). Note, however, that these classes can't be used with `obcClassName` because they don't extend `OrderByComparator<AssetEntry>`. |
